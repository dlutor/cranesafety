...
import os
import time
from transform_model import sqs2sqs__att
from transformer.batch import subsequent_mask
from torch.optim import Adam,SGD, RMSprop, Adagrad
# from transformer.noam_opt import NoamOpt
from transformer.noam_opt_o import NoamOpt
import numpy as np
import scipy.io
import json
import pickle
...
def main():
    ...
    parser.add_argument('--name', type=str, default="hotel")
    parser.add_argument('--factor', type=float, default=0.01)
    parser.add_argument('--save_step', type=int, default=10)
    parser.add_argument('--warmup', type=int, default=2)
    parser.add_argument('--evaluate', type=bool, default=True)
    parser.add_argument('--model_pth', type=str, default='models/pretrained.pth')
    ...
    model = sqs2sqs__att(2,2,7,12,16,16,16,4,1,4,4,0.2).to(device)
    best_mad = 100
    best_model_path = f'models/Individual/{args.name}/best.pth'
    best_result={'epoch':0,'mad':0,'fad':0}
    if args.model_pth:
        model.load_state_dict(torch.load(f'{args.model_pth}'))
        print(f'load model: {args.model_pth}')

    tr_dl = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=0)
    val_dl = torch.utils.data.DataLoader(val_dataset, batch_size=128, shuffle=True, num_workers=0)
    test_dl = torch.utils.data.DataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=0)
    #args.batch_size
    ...
    while epoch<args.max_epoch:
        ...
        for id_b,batch in enumerate(tr_dl):
            ...
            loss = F.pairwise_distance(pred,target).mean()
            loss.backward()
            optim.step()
            print("train epoch %03i/%03i  batch %04i / %04i loss: %7.4f" % (epoch, args.max_epoch, id_b, len(tr_dl), loss.item()))
            epoch_loss += loss.item()
            log.add_scalar('lr', optim._rate, epoch * len(tr_dl)+ id_b)
            # log.add_scalar('lr_step', optim._rate, optim._step)
        #sched.step()
        ...
        with torch.no_grad():
            ...
            for id_b, batch in enumerate(val_dl):
                ...
                dec_inp = model.evaluate(inp)
                preds_tr_b = (dec_inp * std.to(device) + mean.to(device)).cpu().numpy().cumsum(1) + batch['src'][:, -1:,0:2].cpu().numpy()
            ...
            if args.evaluate:
                ...
                pr = np.concatenate(pr, 0)
                mad, fad, errs = baselineUtils.distance_metrics(gt, pr)
                if best_result['epoch'] == epoch:
                    best_result['mad'] = mad
                    best_result['fad'] = fad
                    print(f'{args.dataset_name}{best_result}')
